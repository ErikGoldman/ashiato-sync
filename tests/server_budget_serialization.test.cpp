...
constexpr std::size_t slight_only_mtu_bytes = 32;
...
struct BudgetSerializationServer {
    ...
    explicit BudgetSerializationServer(
        std::size_t budget_bytes,
        std::size_t refusal_bound,
        std::size_t mtu_bytes = 4096U) {
        ...
        options.mtu_bytes = mtu_bytes;
        options.entity_replication_decision_interval_frames = 0;
        ...
        options.entity_replication_decider = [this](
            ...
            decision.priority = context.entity == slight ? 1.0f : 100.0f;
            return decision;
        };
...
TEST_CASE("a starved client serializes every dirty candidate the budget then refuses") {
    BudgetSerializationServer starved(starved_budget_bytes, unbounded_refusals);
    starved.tick();
...
TEST_CASE("a refusal bound caps what a starved client serializes and throws away") {
    BudgetSerializationServer starved(starved_budget_bytes, 4U);
    starved.tick();
    ...
    // the trade this option exposes.
    REQUIRE(starved.records == 1);
}

TEST_CASE("records exceeding MTU do not consume the budget refusal bound") {
    BudgetSerializationServer mtu_limited(64U * 1024U, 1U, slight_only_mtu_bytes);
    mtu_limited.tick();

    REQUIRE(mtu_limited.records == 1);
    REQUIRE(probe_quantize_calls() == bulky_entity_count + 1U);
}
...
TEST_CASE("a refusal bound changes nothing for a client whose budget is not starved") {
    ...
    // strictest setting.
    BudgetSerializationServer generous(64U * 1024U, 1U);
    generous.tick();
